...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['truck-driver', 'business-owner', 'shipper', 'freight-broker'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<FreightBrokerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/freight-brokers` : null),
    () => getFreightBrokers({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
...

    {
      ...
      path: isTenantUser ? `/freight-brokers/view/${data?.[0]?.id}` : '/freight-brokers',
